...
cap = cv2.VideoCapture("Test Videos/test1.mp4")

target_classes = ['driver', 'forklift', 'person']

forklift_last_position = None
forklift_last_time = None
motion_threshold = 3
...
def detect_movement_and_speed(name, center_x, center_y, detections, frame, pts):
    global forklift_last_position, forklift_last_time
            ...
            if inside_polygon((center_x, center_y), np.array([pts])):
                if name == 'forklift':
                    ...
                    if forklift_moving:
                        ...
                        speed_kmh = speed * 3.6  # แปลงจากเมตรต่อวินาทีเป็นกิโลเมตรต่อชั่วโมง
                        cv2.putText(frame, f"Forklift Speed: {speed_kmh:.2f} km/h", (720, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
